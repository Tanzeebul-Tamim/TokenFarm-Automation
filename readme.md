...
TokenFarm is a Python automation tool designed to streamline the process of claiming tokens for multiple web accounts. It leverages Selenium with undetected ChromeDriver to interact with a web interface and automate token-claiming tasks across multiple browser profiles simultaneously.

---

## ⚠️ Disclaimer

This project is for educational purposes only. **Automated interaction with web services may violate Terms of Service.** Use responsibly and in accordance with the terms of service of any platforms you interact with. The developers assume no liability for misuse.
...
- [Telegram Notification Example](#-telegram-notification-example)
- [Sample Log](#-sample-log)
- [Setup](#-setup)
- [Automation with Cron Jobs](#-automation-with-cron-jobs-optional)
- [Helpful Aliases](#️-helpful-aliases-optional-but-recommended)
...
A log file [`log.txt`](./log.txt) is automatically generated by the bot each time it runs (including when triggered by the cron job commands above). It records the progress and results of each token claim session.
...
- `BALANCE_CONTAINER_CLASS`, `BALANCE_TEXT_CLASS`, `CLAIM_BUTTON_CLASS`, `CLAIM_BUTTON_TEXT`, `CLAIMED_STATUS_TEXT`, and `LOGIN_TEXT_FRAGMENT`: Selector values used by the bot to find balance, claim, and login UI elements. If the website layout changes, update these values in `.env` instead of editing the Python code.
...
> **Note:** Most Linux distributions include `cron` by default. If you are on macOS, `cron` is also available. On Windows, you can use Task Scheduler or install a cron-like tool such as [cronw](https://github.com/xen0bit/cronw).
...
- **Scheduled Run at a Specific Time (e.g., 11:55 AM daily):**
    ...
    - `DISPLAY=:0` allows Chrome to run in a graphical environment (required for non-headless mode or when testing visually). For seamless background execution (no popups), you can remove `DISPLAY=:0`.
